...
import { ReportType } from '@/app/index';
...
export default function ReportTypeFilter({ value, onChange }: ReportTypeFilterProps) {
  const [isOpen, setIsOpen] = useState(false);

  const options = [
    { value: 'simple', label: 'Обычный отчет (месяц)' },
    { value: 'comparative', label: 'Сравнительный (месяц с месяцем)' },
  ];
  ...
  return (
    <div className="relative">
      <label className="text-sm text-secondary mb-2 block">Вид отчета</label>
      <button
        ...
        <div className="absolute z-10 mt-2 w-full bg-white rounded-2xl border-2 border-accent shadow-lg overflow-hidden">
          {options.map((option) => (
            <button
              key={option.value}
              onClick={() => {
                onChange(option.value as ReportType);
                setIsOpen(false);
              }}
